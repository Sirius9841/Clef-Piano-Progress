...
export function TechniqueHistoryPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Read-only local history" title={record.exercise.title} description={`Performed ${new Date(record.performedAt).toLocaleString()}`} action={<StatusPill tone="neutral"><History size={13} /> Frozen V1 snapshot</StatusPill>} /><Link className="button ghost" to="/technique"><ChevronLeft size={14} /> Technique Lab</Link><section className="panel challenge-profile"><strong>Exact challenge</strong><span>{record.exercise.challenge.targetTempoBpm} BPM · {record.exercise.challenge.eventCount} events · instance {record.exerciseInstanceId} · {record.novelty.firstSavedAttempt ? 'first saved encounter' : 'repeat encounter'}</span></section><section className="panel notation-panel"><OsmdScoreRenderer musicXmlText={record.exercise.generatedMusicXml} zoom={.78} /></section><TechniqueResultPanel result={record.techniqueAnalysis} /><p className="honest-notice">Historical Technique evidence is displayed from its saved snapshots. It is never silently regenerated with current engines.</p></div>
}
