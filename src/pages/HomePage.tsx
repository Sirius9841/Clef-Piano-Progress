...
import { currentScorePersonalBestEvents, latestAttemptForScoreVersion, latestCurrentAttemptForRepertoireItem } from '../components/currentScorePresentation'
      ...
      {data && featured.length > 0 && <>
        ...
        <section className="hero-grid reveal delay-1">
          <div className="panel continue-panel">
            ...
            <div className="continue-list">{featured.map((item, index) => { const { work, arrangement, scoreVersion } = item; const latestAttempt = latestCurrentAttemptForRepertoireItem(item, data.allAttempts); return <Link to={`/repertoire/${arrangement.id}`} className="continue-item" key={arrangement.id}><div className={`artwork artwork-${index + 1}`}><span>{work.title.split(' ').slice(0, 2).map((word) => word[0]).join('')}</span><i /></div><div className="piece-copy"><strong>{work.title}</strong><span>{work.composer} · {arrangement.name}</span><small>{latestAttempt ? `Score v${scoreVersion.version} · ${latestAttempt.gradingScope === 'full-plan' ? 'Full score' : 'Played section'} · ${Math.round(latestAttempt.practiceSpeedMultiplier * 100)}%` : `Score v${scoreVersion.version} · No current-score take yet`}</small></div><div className="recent-score"><span>Notes</span><strong>{formatPercent(latestAttempt?.notes ?? null)}</strong></div><span className="play-circle"><Play size={17} fill="currentColor" /></span></Link> })}</div>
          </div>
