...
export function ProgressPage() {
  ...
  const state = useRepositoryQuery<ProgressPageData>(async (repository) => {
    ...
    const [snapshot, techniqueSummaries] = await Promise.all([repository.getProgress(range), repository.listTechniqueAttemptSummaries()])
    return { snapshot, skills: deriveAllSkillRatings(techniqueSummaries, asOf) }
  }, `progress:${range}`)
  ...
  return (
    <div className="page">
      ...
      {snapshot && <>
        ...
        <SkillRatingsPanel skills={skills} />
      </>}
    </div>
  )
}
