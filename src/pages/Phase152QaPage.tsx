...
import { buildPerformanceResults } from '../features/performance-results/buildPerformanceResults'
...
type Scenario = 'beginning' | 'middle' | 'ambiguous'
...
export function Phase152QaPage() {
  ...
  const analysis = useMemo(() => {
    const recording = makeRecording(sourceAttacks(scenario), { id: `qa-recording:${scenario}`, planId: plan.id })
    const localizationHint: ScoreRegionLocalizationHint = confirmed
      ...
      : hintFor(scenario)
    const alignment = alignPerformance(plan, recording, { localizationHint })
    if (!alignment.localization?.takeRegion) return { recording, alignment, results: null }
    const notes = gradeNotes({ expectedPlan: plan, recording, alignment, options: { gradingScope: 'aligned-span' } })
    const timing = analyzeTiming({ expectedPlan: plan, recording, alignment, noteGrading: notes })
    const results = buildPerformanceResults({ normalizedScore: score, expectedPlan: plan, alignment, noteGrading: notes, timingAnalysis: timing })
  ...
  return <div className="page phase152-qa-page">
    ...
    <TakeReview alignment={analysis.alignment} recording={analysis.recording} practiceSpeed={1} results={analysis.results} expression={null} pedal={null} voicing={null} onConfirmRegion={setConfirmed} onHighlightChange={() => undefined} />
