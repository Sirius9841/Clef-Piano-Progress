...
import { boundedRecentRepertoire, latestAttemptForScoreVersion } from '../components/currentScorePresentation'
...
export function RepertoirePage() {
  ...
  const recent = boundedRecentRepertoire(filtered)
      ...
      {queryState.status === 'ready' && items.length > 0 && <>
        ...
        <div className={`repertoire-grid ${view} reveal delay-2`}>
          {recent.map((item, index) => {
            const { work, arrangement, repertoire, scoreVersion, sessionCount, lastPracticedAt } = item
            const latestAttempt = latestAttemptForScoreVersion(attempts.filter((attempt) => attempt.arrangementId === arrangement.id), scoreVersion.id)
            return (
            <article className="repertoire-card" key={arrangement.id}>
              ...
              <div className="repertoire-card-body">
                ...
                {latestAttempt ? <div className="card-metrics"><div><span>Latest current-score Notes</span><strong>{formatPercent(latestAttempt.notes)}</strong></div><div><span>Rhythm / Tempo</span><strong>{formatPercent(latestAttempt.rhythm)} · {formatPercent(latestAttempt.tempo)}</strong></div></div> : <div className="take-empty current-score-empty">No current-score result</div>}
        ...
        <section className="repertoire-ledger panel"><SectionHeading title="Full repertoire ledger" subtitle="Every arrangement matching the current search and status filter" /><div className="ledger-table" role="table" aria-label="Full repertoire"><div className="ledger-head" role="row"><span>Work / Arrangement</span><span>Status</span><span>Current score</span><span>Last practiced</span><span>Practice</span></div>{filtered.map((item) => <div className="ledger-row" role="row" key={item.arrangement.id}><Link to={`/repertoire/${item.arrangement.id}`}><strong>{item.work.title}</strong><small>{item.arrangement.name}</small></Link><span>{item.repertoire.status}</span><code>v{item.scoreVersion.version}</code><span>{formatLastPracticed(item.lastPracticedAt)}</span><Link className="text-link" to={`/repertoire/${item.arrangement.id}`}>Open <Play /></Link></div>)}</div></section>
