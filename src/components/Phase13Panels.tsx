...
export function SkillRatingsPanel({ skills }: { readonly skills: readonly SkillRating[] }) {
  return <section className="panel skill-model-panel reveal delay-3">
    ...
    <div className="skill-rating-grid">{skills.map((skill) => {
      ...
      return <article className="skill-rating-card" key={skill.moduleId}>
        ...
        <small>{skill.qualityEstimate === null ? legacyOnly ? 'Historical Technique evidence is preserved, but this model requires the current 1.1.1 / 1.1.2 evidence pair.' : 'Needs current Technique evidence.' : `${skill.eligibleContextCount} context${skill.eligibleContextCount === 1 ? '' : 's'} · ${skill.eligibleAttemptCount} eligible take${skill.eligibleAttemptCount === 1 ? '' : 's'}`}</small>
    ...
    <details className="model-diagnostics"><summary>Skill model details</summary><span>skill-model-1.0.0 · current Technique engine pair only · no overall pianist score</span></details>
  </section>
}
...
export function ArrangementMasteryPanel({ mastery }: { readonly mastery: ArrangementMastery }) {
  ...
  return <section className="panel mastery-model-panel reveal delay-2">
    ...
    {mastery.status === 'unestablished' ? <div className="mastery-empty"><strong>Mastery needs current full-score evidence</strong><p>{oldVersionOnly ? 'Earlier ScoreVersions have preserved history, but their evidence does not carry into this score realization.' : 'Save reliable or limited full-score Notes, Rhythm, and Tempo results for this ScoreVersion.'}</p></div> : <div className="mastery-component-grid">
      ...
      <article><span>Demonstrated speed</span><strong>{mastery.demonstratedSpeedMultiplier === null ? 'Needs repetition' : `${Math.round(mastery.demonstratedSpeedMultiplier * 100)}%`}</strong><small>{mastery.demonstratedSpeedMultiplier === null ? 'Two qualifying full takes required' : `${mastery.demonstratedSpeedSessionCount} supporting session${mastery.demonstratedSpeedSessionCount === 1 ? '' : 's'}`}</small></article>
      ...
      <article><span>Last evidence</span><strong>{formatDate(mastery.lastEvidenceAt, true)}</strong><small>{mastery.eligibleAttemptCount} eligible full-score take{mastery.eligibleAttemptCount === 1 ? '' : 's'} · {mastery.distinctSessionCount} session{mastery.distinctSessionCount === 1 ? '' : 's'}</small></article>
    </div>}
    <details className="model-diagnostics"><summary>Mastery model details</summary><span>mastery-model-1.0.0 · current ScoreVersion · Notes, Rhythm, Tempo, repetition, consistency, and recency only</span></details>
  </section>
}
