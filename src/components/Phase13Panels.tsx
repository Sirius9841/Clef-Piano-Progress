...
function breadthLabel(skill: SkillRating): string {
  ...
  const subdivisions = `subdivisions ${envelope.subdivisions.join(' / ') || '—'}`
  const starts = `starts ${envelope.startingTonics.map(tonicLabel).join(' / ') || '—'}`
  if (skill.moduleId === 'sight-reading') return `${envelope.distinctFirstPassExerciseInstances} first-pass instance${envelope.distinctFirstPassExerciseInstances === 1 ? '' : 's'} · ${envelope.tonics.length} key${envelope.tonics.length === 1 ? '' : 's'} · ${subdivisions} · ${bpm}`
  if (skill.moduleId === 'scales' || skill.moduleId === 'arpeggios') return `${envelope.tonics.map(tonicLabel).join(', ') || 'No keys'} · ${envelope.modes.join(' / ') || 'no modes'} · ${envelope.octaveSpans.join(' / ') || '—'} octave · ${subdivisions} · ${bpm}`
  if (skill.moduleId === 'chord-fluency') return `${envelope.tonics.length} key${envelope.tonics.length === 1 ? '' : 's'} · inversions ${envelope.chordInversions.join(' / ') || '—'} · ${subdivisions} · ${bpm}`
  if (skill.moduleId === 'keyboard-jumps') return `${envelope.jumpDistancesSemitones.join(' / ') || '—'} semitones · ${starts} · ${subdivisions} · ${bpm}`
  if (skill.moduleId === 'tempo-control') return `${envelope.tempoShapes.join(' / ') || 'no shapes'} · ${starts} · ${subdivisions} · ${bpm}`
  return `${starts} · ${subdivisions} · ${bpm}`
}
  ...
  return <section className="panel skill-model-panel reveal delay-3">
    ...
    <div className="skill-rating-grid">{skills.map((skill) => {
      ...
      return <article className="skill-rating-card" key={skill.moduleId}>
        ...
        <small>{skill.qualityEstimate === null ? legacyOnly ? 'Historical Technique evidence is preserved, but this model requires the current 1.1.1 / 1.1.2 evidence pair.' : 'Needs current Technique evidence.' : `${skill.modelEvidenceAttemptCount} model-window take${skill.modelEvidenceAttemptCount === 1 ? '' : 's'} · ${skill.eligibleAttemptCount} eligible take${skill.eligibleAttemptCount === 1 ? '' : 's'} · ${skill.eligibleContextCount} context${skill.eligibleContextCount === 1 ? '' : 's'} · ${skill.challengeEnvelope.distinctTemplateCount} template${skill.challengeEnvelope.distinctTemplateCount === 1 ? '' : 's'}`}</small>
...
export function ArrangementMasteryPanel({ mastery }: { readonly mastery: ArrangementMastery }) {
  ...
  const speedDetail = mastery.demonstratedSpeedStatus === 'established'
    ? `${mastery.demonstratedSpeedQualifyingAttemptCount} qualifying takes · ${mastery.demonstratedSpeedSessionCount} raw session${mastery.demonstratedSpeedSessionCount === 1 ? '' : 's'} · current session support ${formatSupport(mastery.demonstratedSpeedEffectiveSessionSupport)} · last demonstrated ${formatDate(mastery.demonstratedSpeedLastEvidenceAt)}`
    : mastery.demonstratedSpeedStatus === 'needs-current-support'
      ? 'Qualifying evidence exists, but current reliability/recency support is not strong enough to establish this speed.'
      : mastery.demonstratedSpeedStatus === 'needs-repetition'
        ? `${mastery.demonstratedSpeedQualifyingAttemptCount} qualifying take${mastery.demonstratedSpeedQualifyingAttemptCount === 1 ? '' : 's'} · repeat at this speed`
  ...
  return <section className="panel mastery-model-panel reveal delay-2">
    ...
    {mastery.status === 'unestablished' ? <div className="mastery-empty"><strong>Mastery needs current full-score evidence</strong><p>{oldVersionOnly ? 'Earlier ScoreVersions have preserved history, but their evidence does not carry into this score realization.' : 'Save reliable or limited full-score Notes, Rhythm, and Tempo results for this ScoreVersion.'}</p></div> : <div className="mastery-component-grid">
      ...
      <article><span>Last evidence</span><strong>{formatDate(mastery.lastEvidenceAt, true)}</strong><small>{mastery.evidenceAttemptIds.length} model-window take{mastery.evidenceAttemptIds.length === 1 ? '' : 's'} · current authority {formatSupport(mastery.effectiveEvidenceSupport)} · {mastery.distinctSessionCount} raw session{mastery.distinctSessionCount === 1 ? '' : 's'}</small></article>
    </div>}
