...
export function analyzeChords(context: TechniqueAnalyzerContext): ModuleAnalysis {
  ...
  const complete = new Set(context.evidence.eventGroups.filter((entry) => entry.noteResult?.classification === 'perfect').map((entry) => entry.expectedGroupId))
  ...
  return { observations: [...accuracy, ...synchronization], findings: widest && mean(synchronization.map((item) => item.score))! < context.options.chordFindingScore ? [`The widest complete-chord attack spread was ${Math.round(widest.value)} ms.`] : [], facets: [
    facet(context, { id: 'chord-accuracy', family: 'pitch', observations: accuracy, eligibleCount: context.exercise.events.length, minimumEvidence: context.options.minimumChordEvents, summary: 'One authored chord event is one complete-or-incomplete accuracy vote.' }),
    facet(context, { id: 'chord-synchronization', family: 'synchronization', observations: synchronization, eligibleCount: complete.size, minimumEvidence: context.options.minimumChordSynchronizationEvents, summary: 'One complete correct chord is one attack-spread vote, independent of chord size.' }),
  ] }
}
