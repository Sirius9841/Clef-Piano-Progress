...
export function analyzeArpeggios(context: TechniqueAnalyzerContext): ModuleAnalysis {
  ...
  return { observations: [...pitch, ...transitions], findings, facets: [
    facet(context, { id: 'note-accuracy', family: 'pitch', observations: pitch, eligibleCount: context.exercise.events.length, minimumEvidence: context.options.minimumNoteEvents, summary: 'One authored arpeggio event is one pitch vote.' }),
    facet(context, { id: 'arpeggio-transition-consistency', family: 'interval-precision', observations: transitions, eligibleCount: Math.max(0, context.exercise.events.length - 1), minimumEvidence: context.options.minimumEvennessIntervals, summary: 'Median-centered consistency across safe broken-chord transitions, including identified register boundaries.' }),
  ] }
}
