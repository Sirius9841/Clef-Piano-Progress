...
export function TechniqueResultPanel({ result }: { result: TechniqueAnalysisResultV1 | TechniqueAnalysisResultV2 }) {
  ...
  return <section className="panel technique-results">
    ...
    <div className="technique-facet-grid">{result.facets.map((facet) => <article key={facet.id}><span>{facet.label}</span><strong>{facet.score === null ? 'Unavailable' : `${facet.score.toFixed(1)}%`}</strong><small>{facet.summary}</small><div className="challenge-line">{facet.evidenceCount}/{facet.eligibleCount} evidence · {Math.round(facet.coverage * 100)}% coverage · {facet.reliability}{'evidenceFamily' in facet ? ` · ${facet.evidenceFamily}` : ''}</div></article>)}</div>
