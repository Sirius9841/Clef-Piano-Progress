...
export function useTimingAnalysis(
  ...
  const analyze = useCallback(async (noteOverride?: NoteGradingResult | null) => {
    ...
    if (!plan || !recording || !alignment || !grading) return null
    const key = resultKey(plan, recording, alignment, grading)
    const currentGeneration = ++generation.current
    setStored({ key, state: { status: 'analyzing' } })
    await new Promise<void>((resolve) => window.setTimeout(resolve, 0))
    try {
      const { analyzeTiming } = await import('./analyzeTiming')
      const result = analyzeTiming({ expectedPlan: plan, recording, alignment, noteGrading: grading })
