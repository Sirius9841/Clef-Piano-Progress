...
export function HistoricalExpressionPanel({ result }: { result: ExpressionAnalysisResult | null }) {
  if (result) return <ExpressionAnalysisPanel analysis={{ status: 'ready', result }} onAnalyze={() => undefined} readOnly />
  return <section className="panel expression-state unavailable historical-expression-unavailable"><div><Sparkles /><span><small>Historical expression</small><strong>Dynamics and Articulation not analyzed</strong><p>This Phase 1–8 snapshot predates expression analysis. Clef will not silently regrade it with a newer engine.</p></span></div></section>
}
