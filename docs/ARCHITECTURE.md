...
After a take stops, Practice can dynamically load the pure alignment engine. Alignment `2.0.0` first proposes contiguous score regions from pitch fingerprints and evaluates them without timing. Beginning or an exact Phase 14 `PlanningSectionIdentity` may resolve a close candidate only when its own structural evidence remains credible. Ambiguous repeated passages expose bounded candidates for user confirmation; divergent takes fail closed. Fine monotonic alignment and affine time fitting run only inside the selected region, producing an immutable `MatchedTakeRegion` with exact group and measure bounds. The engine never updates recording or score state, and analysis remains session-only. See `PERFORMANCE_LOCALIZATION.md`.
...
Phase 15.2 changes Alignment from `1.0.0` to `2.0.0` and Timing Analysis from `1.0.0` to `1.1.0`. `PerformanceAttemptV4` remains the current record family because the added localization and timing diagnostics are version-gated fields inside existing immutable engine snapshots, not new attempt-owned evidence. IndexedDB stays at schema `4`; no migration is required. Legacy engine snapshots remain readable and frozen and are never upgraded in place.
