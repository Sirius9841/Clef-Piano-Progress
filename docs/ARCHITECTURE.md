...
Technique Lab is a separate Phase 12 domain. Deterministic specs compile to generated MusicXML, the canonical normalized score, and an ExpectedPerformancePlan; real MIDI then reuses alignment, note grading, and timing before Technique-only facet analysis. TechniqueAttempt V1 and its lightweight summary live in dedicated schema-4 stores, so they do not contaminate Work, Arrangement, ScoreVersion, PracticeSession, PerformanceAttempt, or repertoire progress. Historical Technique views display frozen snapshots without reanalysis. Library remains an honest metadata-only preview. See `TECHNIQUE_LAB.md`.
