...
Technique Lab is a separate Phase 12 domain. Deterministic specs compile to generated MusicXML, the canonical normalized score, and an ExpectedPerformancePlan; real MIDI then reuses alignment, note grading, and timing before Technique-only facet analysis. Frozen TechniqueAttempt V1 (`1.0.0`) and V2 (`1.1.0`) records and versioned summaries live in dedicated schema-4 stores, so they do not contaminate Work, Arrangement, ScoreVersion, PracticeSession, PerformanceAttempt, or repertoire progress. V2 prepares only adjacent perfect-note interval evidence, preserves per-observation provenance, separates actual event coverage from reached span, and dispatches independent module-specific facet analyzers. Historical views render their exact saved version without reanalysis. See `TECHNIQUE_LAB.md`.
