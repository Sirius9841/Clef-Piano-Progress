...
Phase 13.1 hardens the two pure, derived read models above persistence summaries. `features/skill-model` consumes only validated Technique summaries, qualifies the exact current engine pair, centralizes template-aware authored challenge identity, caps each context to its latest three model attempts, and separates gentle quality continuity from no-floor confidence authority. `features/mastery-model` consumes only lightweight performance summaries for one Arrangement and its current ScoreVersion; demonstrated speed has recency/reliability authority from its own exact supporting bucket, while overall recency and confidence reflect the full bounded evidence distribution and cap each session's influence. Neither model imports React, IndexedDB, OSMD, MIDI, or raw analysis engines; neither creates a store or mutates frozen history. See `SKILL_MODEL.md` and `MASTERY_MODEL.md`.
