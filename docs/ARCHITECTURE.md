...
Phase 13 adds two pure, derived read models above persistence summaries. `features/skill-model` consumes only validated Technique summaries, qualifies the exact current engine pair, aggregates attempts into typed challenge contexts before deriving eight independent current ratings, and exposes breadth/confidence beside quality. `features/mastery-model` consumes only lightweight performance summaries for one Arrangement and its current ScoreVersion, deriving separate control, demonstrated-speed, consistency, recency, and confidence evidence. Neither model imports React, IndexedDB, OSMD, or raw analysis engines; neither creates a store or mutates frozen history. See `SKILL_MODEL.md` and `MASTERY_MODEL.md`.
