...
Phase 14 adds a third derived read-model boundary. `preparePracticePlanningContext` filters lightweight summaries to one Arrangement/current ScoreVersion, latest eight sessions, and three attempts per session before loading at most 24 frozen attempts through the repository. The pure planner builds exact source-measure section histories, session-capped authority, speed-aware actions, Mastery verification, independent Technique targets, and an optional bounded session composition. It neither imports React/OSMD nor calls IndexedDB, never regrades or mutates evidence, and preserves Phase 7 Practice Priority unchanged. See `PRACTICE_PLANNING.md`.
