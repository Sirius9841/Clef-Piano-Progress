...
Phase 10 saves use `PerformanceAttemptRecordV3` and preserve the exact Pedal result and engine version beside the exact V2 expression snapshot. Phase 11 V4 saves retain the same exact Pedal snapshot beside Voicing and reference comparison. Frozen V3 `pedal-analysis-1.0.0` records remain readable without additive 1.1 anchor/channel/coverage diagnostics, and frozen `pedal-analysis-1.1.0` records retain their original timing semantics; neither is reanalyzed. New records use `pedal-analysis-1.1.1` with the existing modern 1.1 result shape. V1/V2 history is shown as “Pedal not analyzed.” Phase 12 later raises IndexedDB to schema 4 only for separate Technique stores; AttemptSummary, personal bests, trends, Home, and Progress remain Notes/Rhythm/Tempo-only.

Reference comparison consumes frozen Pedal event evidence only when both snapshots have compatible semantics. Timing differences are expressed relative to each take's own local musical anchor. A legacy 1.0 snapshot is not silently compared as though it had modern local-anchor semantics.
...
Phase 10 covers authored damper notation and MIDI CC64 only. It does not grade una corda/soft pedal, sostenuto, acoustic resonance, pedal noise, half-pedal depth, instrument polarity overrides, audio, voicing, or overall musicality. Phase 11 may neutrally compare compatible frozen Pedal evidence between saved takes; the reference is not a ground truth.
