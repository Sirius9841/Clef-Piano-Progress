...
Phase 12 schema 4 additively creates `techniqueAttempts` and `techniqueAttemptSummaries`. Both index `performedAt`, `moduleId`, `templateId`, and `exerciseInstanceId`. A Technique save writes full and summary records in one transaction and retries idempotently by ID. Reads defensively validate snapshot provenance. Existing PerformanceAttempt V1–V4 records are preserved and never rewritten; their summaries remain Notes/Rhythm/Tempo-only and unchanged. Clear-all includes both new stores.
