...
Phase 13 adds no stores, schema change, PerformanceAttempt version, TechniqueAttempt version, or mutable counters. Skill and Mastery are deterministic read-time projections over `listTechniqueAttemptSummaries()` and `listAttemptSummaries()`. Summary projections remain rebuildable accelerators; immutable attempt snapshots remain authoritative history.
