...
Phase 11 stores musical history in browser IndexedDB behind `PianoProgressRepository`. Domain and React code never issue raw IndexedDB requests. The adapter uses schema version `3`: v2 preserved ScoreVersion part selection and v3 added the session `endedAt` range index. Phase 11 adds only optional fields to existing object values, so no database migration is needed. Migrations are ordered and never rewrite immutable historical identity.
...
New V4 attempts use `reference-comparison-1.1.0`. Existing V4 `reference-comparison-1.0.0` attempts remain readable as exact frozen history. The attempt schema remains V4 and `PERSISTENCE_SCHEMA_VERSION` remains 3; this hardening changes neither stores nor indexes.

Neither object-format extension nor the additive Phase 10.1 result diagnostics nor the Phase 10.2 engine patch creates a store or index, so the IndexedDB schema remains version 3. V1 records are not rewritten or reanalyzed and report Expression, Pedal, Voicing, and reference comparison as not analyzed where absent. V2 preserves its exact Expression result, V3 retains its saved Pedal semantics, and V4 preserves its exact Phase 11 snapshots. Attempt summaries remain Notes/Rhythm/Tempo-only and are unchanged.
