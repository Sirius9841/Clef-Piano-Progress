...
The current progress model deliberately excludes Mastery, clean playable tempo, Dynamics, Articulation, Pedal, and Voicing personal bests/trends, Technique Lab ratings, reference-similarity trends, and an overall performance score. Phase 9 expression, Phase 10 pedal, and Phase 11 Voicing/reference snapshots remain visible per attempt but are not headline longitudinal metrics. A selected reference is an interpretive comparison preference, never a record to beat.
